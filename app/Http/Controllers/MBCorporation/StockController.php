            ...
            if ($request->item_id) {
                ...
                if (count($deletedProducts) > 0 && $deletedProducts != null) {
                    ...
                    foreach ($deletedProducts as $key => $productId) {
                        StockHistory::where('stockable_id', $data->id)->where("stockable_type", "App\StockTransfer")->where('item_id', $productId)->where('godown_id', $data->location_form)->delete();
                        StockHistory::where('stockable_id', $data->id)->where("stockable_type", "App\StockTransfer")->where('item_id', $productId)->where('godown_id', $data->location_to)->delete();
                        ...
                        $old_p_Stock_qty = StockDetail::where('item_id', $productId)->where('godown_id', $data->location_to)->first();
                        $new_p_stock_qty = $old_p_Stock_qty->qty -  $demoProductAddOnVoucher->qty;
                        $old_p_Stock_qty->update([
                            'qty' => $new_p_stock_qty,
                        ]);
            ...
            if ($moves->isNotEmpty()) {
                foreach ($moves as $for_name_row) {
                    ...
                    if ($itemCount) {
                        ...
                        ItemCount::updateOrCreate(['item_id' => $for_name_row->id], [
                            'generated_qty' => $for_name_row->qty ?? 0,
            ...
            if ($removes->isNotEmpty()) {
                ...
                foreach ($removes as $for_name_row) {
                    ...
                    if ($itemCount) {
                        ...
                        ItemCount::updateOrCreate(['item_id' => $for_name_row->id], [
                            'consumed_qty' => $for_name_row->qty ?? 0,
                            'avg_purchase' => (new Product)->average_price($for_name_row->item_id)
                        ]);
                    }
                }
