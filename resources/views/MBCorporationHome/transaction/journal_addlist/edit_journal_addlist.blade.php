                                    ...
                                    <div class="col-md-12">
                                        <table class="table table-bordered" id="journal_list">
                                            ...
                                            <tfoot>
                                                <tr>
                                                    <td>
                                                        <select id="account_name" class="select2 form-control"
                                                            style="width: 100%" data-placeholder="Select a Account Ledger">
                                                        </select>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <button type="button" onclick="addDemoContraJournal()"
...
@push('js')
    <script>
        ...
        function addDemoContraJournal() {
            ...
            htmlData += "<td> <input class='drcr_text form-control' readonly  type ='text' name='new_drcr_text[]' value='" +
                drcrText + "' /></td>"
            htmlData +=
                "<td><input class='form-control' type ='number' placeholder='Amount' min='0' name='new_amount[]' value='" +
                amount + "' /> </td>"
            htmlData += "<td><input class='form-control' type ='text' placeholder='Note' name='new_note[]' value='" + note +
                "' /></td>"
            htmlData +=
                "<td><button class='btn btn-sm btn-danger' onclick='delete_data(this)'><i class='fa fa-trash'></i></button></td>"
            htmlData += "</tr>";
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(key, value) {
                        ...
                        htmlData += "<input type='hidden' name='new_account_id[]' value='" + value
                            .ledger_id + "'/>"
                        htmlData += "<td>" + value.ledger.account_name + "</td>"
                        htmlData +=
                            "<td><input class='drcr_text form-control' readonly  type ='text' name='new_drcr_text[]' value='" +
                            value.drcr + "' /></td>"
                        htmlData +=
                            "<td><input class='form-control' min='0' placeholder='Amount'  type ='number' name='new_amount[]' value='" +
                            value.amount + "' /> </td>"
                        htmlData +=
                            "<td><input class='form-control' placeholder='Note'  type ='text' name='new_note[]' value='" +
                            value.note + "' /></td>"
                        htmlData += "<td>"
                        htmlData +=
                            "<button type='button' class='btn btn-sm btn-danger' onclick='delete_data(this)'><i class='fa fa-trash'></i></button>"
                        htmlData += "</td>"
                        htmlData += "</tr>";
                    });
                    $('#journal_list tbody').append(htmlData);
