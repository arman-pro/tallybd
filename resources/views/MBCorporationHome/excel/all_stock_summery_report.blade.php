...
<table>
    <thead>
        ...
        <tr>
            <th  colspan="6">Stock Summery Report</th>
        </tr>
        <tr>
    ...
    <tr>
        ...
        <td>{{number_format($averagePrice, 2)}}</td>
        <td>{{number_format($totalCount, 2)}}</td>
        <td>{{new_number_format($saleAvg*$totalCount , 2)}}</td>
        <td>{{new_number_format($averagePrice*$totalCount, 2)}}</td>
    </tr>
    @endif
    @endforeach
    </tbody>
    <tfoot>
        <tr>
            <td colspan="4">{{number_format($all_total_qty, 2)}}</td>
            <td>{{new_number_format($all_total_sales_price, 2)}}</td>
            <td>{{new_number_format($all_total_pur_price, 2)}}</td>
        </tr>
    </tfoot>
</table>